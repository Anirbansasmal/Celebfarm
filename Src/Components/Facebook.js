...
export default class Facebook extends Component {
  ...
  async onNavigationStateChange(webViewState) {
    ...
    if (
    //   webViewState.title === 'Facebook' &&
      webViewState.url === 'https://www.facebook.com/v21.0/dialog/oauth'
    ) {
      ...
      if (results.access_token) {
        ...
        if (responseType === 'code' && !appSecret) {
          ...
          let http = axios.create({
            baseURL: 'https://graph.facebook.com/v21.0/oauth/access_token',
            headers: headers,
...
//   }
  ...
  renderWebview() {
    ...
    let ig_uri = `https://api.instagram.com/oauth/authorize/?client_id=&redirect_uri=${redirectUrl}&response_type=${responseType}&scope=${scopes.join(',')}`;
let face_id=`https://www.facebook.com/v21.0/dialog/oauth?client_id=${appId}&redirect_uri${redirectUrl}&response_type=code&scope=${scopes.join(',')}`
    return (
      <WebView
        {...this.props}
        key={key}
